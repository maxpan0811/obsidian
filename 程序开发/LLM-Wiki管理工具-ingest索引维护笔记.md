...
### 2026-07-15 全库索引扩展 + 速度分析

#### 全库索引
将 FAISS 索引从仅 `wiki/sources/`（23,668 篇）扩展到 Obsidian 全库（71,497 篇）。

**改动：**
- `_scan_source_files()`：从单目录扫描改为 vault 递归扫描
- 排除目录：`.obsidian`, `.git`, `.trash`, `attachments`, `wiki/vector_store`
- `_load_cache()`：v3 迁移——纯文件名 key → vault 相对路径 key（`人工智能.md` → `wiki/sources/人工智能.md`）
- `_extract_text()`：兼容无 YAML frontmatter 的文件
- `wiki_vector_query.py`：`rel_path` 改为 vault 相对路径

**性能（全库 47,819 篇新增）：**
- 速度：5.1 files/s（比 wiki/sources 的 2.0 快，因为其他目录笔记更短）
- ETA：~2.5 小时
- 内存：1.2GB（稳定）
...
| 环节 | 耗时 | 占比 |
|------|------|------|
| Ollama 纯编码 | 0.4s/批 (80 sentences/s) | 理论极限 27 files/s |
| 文件读取+分块 | < 5ms/批 | 可忽略 |
| Cache 保存 | 464ms/100文件 | 小 |
| 实际速度 | 5.1 files/s | 稳态 |

**结论：不值得进一步优化。** 全库 6.5h → 实际 2.5h，已从旧方案 34h（只跑了一半）提升 50×+。

#### Swap 分析
当前 Swap 6.5GB 是旧 34 小时构建的残留，主要来自 Spotlight（2.7GB）和印象笔记（2.4GB）。当前进程 1.2GB 正常。系统可用内存 84%，健康。

#### 当前状态 (2026-07-15 09:01)
- 进度：960/47,819 (2.0%)
- 会话 ID：`f52edae5-dba6-4311-ae70-49d13bf7f58a`
- 终端命令：`nohup python3 -u wiki_faiss_build.py --incremental > /tmp/faiss_build.log 2>&1 &`
