...
本次解决了一个长期存在的 bug：部分微信公众号文章图片无法抓取（0 图）。

### 根因分析

用 CDP 模式保存"强烈建议大家都去看语文纪录片"时，18 张图全部遗漏。

**问题 1：walker 和 fallback 都只查 `#js_content`**
`extract_content_parts()` walker 和 `page.evaluate` fallback 都只查询 `document.getElementById('js_content')`。
但实测这篇文章 18 张图中有 17 张在 `#js_content` 容器之外（微信公众号不同模板的结构差异，图片可能分布在 `rich_media_content` 等外层容器）。

**问题 2：fallback 触发条件过严**
原条件：`if not walk_found_content:` — 只要 walker 找到了文本块（哪怕 0 图），fallback 就被跳过。
结果：walker 找到了 33 个纯文本块 → `walk_found_content = True` → fallback 不运行 → 0 图。

**问题 3：懒加载 `data-src` 未注入**
部分公众号文章 `<img>` 标签用 `data-src="..."` 替代 `src`。浏览器端需要在内容提取前主动注入 `img.src = data_src`。

### 修复

| 修复 | 文件 | 效果 |
|------|------|------|
| fallback 全文搜索 | `save_articles_via_cdp.py` | `querySelectorAll('img')` 替代 `#js_content` |
| 触发条件放宽 | `save_articles_via_cdp.py` | `or len(img_srcs)==0` |
| data-src 注入 | `save_articles_via_cdp.py` | 页面加载后注入，处理懒加载 |
| 非图片 MIME 过滤 | `save_articles_via_cdp.py` | 跳过 text/html 的伪图片 |

### 验证

测试文章"强烈建议大家都去看语文纪录片"：
- 修复前：0 图片
- 修复后：15 图发现，14 图有效（1 个 text/html 非图片经 MIME 过滤跳过）
- 去重：清理 2 篇残影版

### 同期其他修复

- ENML `&` 实体转义（`&mid=` → `&amp;mid=`）
- 原文链接移至标题下方（`paragraphs.insert(1, link_div)`）

---

## 微信 Bot 集成 · 2026-07-14

**系统序列号**：`019f56b2-620a-7d43-9679-da800d34bab6`

### 最终方案

弃用 Shortcut+iCloud Drive 方案，改用 OpenClaw 官方 WeChat Connect 插件（`@tencent-weixin/openclaw-weixin`），
通过腾讯 ilink API 实现微信 ↔ Codex 的双向消息通道。

### 工作流程

```
微信 → 复制文章链接 → 粘贴到 bot 对话
  → ilink API → OpenClaw Gateway → Codex 接收消息
  → 自动提取 URL → CDP 保存 → 印象笔记工作篮
```
...
| 步骤 | 命令 |
|------|------|
| 安装插件 | `clawhub install wechat` |
| 登录配对 | `openclaw channels login --channel openclaw-weixin` |
| 配置白名单 | `openclaw config set channels.openclaw-weixin.dmPolicy allowlist` |
| 发消息测试 | `openclaw message send --channel openclaw-weixin --target <userId> --message <text>` |

### 已清理的废弃方案

- iOS Shortcut "Forward to Codex" — 不再需要
- iCloud Drive CodexInbox/wechat-articles.md — 已删除
- process_wechat_inbox.py — 已删除
- Obsidian 笔记 "微信转发到Codex收件箱" — 已删除
- Memory 记录 — 已清理

### 同日 CDP 修复（配套依赖）

本次全链路验证中还修复了 CDP 图片抓取的多个 bug：
1. 图片搜索从 `#js_content` 改为全文搜索
2. fallback 触发条件放宽（`or len(img_srcs)==0`）
3. `data-src` 懒加载注入
4. `&` XML 实体转义
5. 非图片 MIME 过滤
6. 原文链接移至标题下方
