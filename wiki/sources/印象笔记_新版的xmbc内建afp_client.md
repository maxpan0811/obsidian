---
title: "新版的xmbc内建afp client"
type: source
created: 2026-07-15
updated: 2026-07-15
source_path: 印象笔记管理工具/新版的xmbc内建afp client.md
tags: [印象笔记]
---

# 新版的xmbc内建afp client

# 新版的xmbc内建afp client --- | | | --- | | 新版的xmbc内建afp client,只需要在添加目录的时候,输入: afp://user\_name:passwor

---

# 新版的xmbc内建afp client

---

|  |
| --- |
| 新版的xmbc内建afp client,只需要在添加目录的时候,输入: afp://user\_name:password@your\_nas\_ip/[/目录] 就可以访问了; 比smb,nfs好多了, smb太慢, xmbc访问nfs经常报无法连接共享. so,如果有支持afp的nas,别用nfs了. |

---

[🌐 原始链接](http://bbs.weiphone.com/read-htm-tid-3029469.html)

[📎 在印象笔记中打开](evernote:///view/207087/s1/4e9143b0-6d49-4d2c-945a-2a8eb9f4cdfb/4e9143b0-6d49-4d2c-945a-2a8eb9f4cdfb/)