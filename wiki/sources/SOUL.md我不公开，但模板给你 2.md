---
title: SOUL我不公开，但模板给你 2
type: source
created: 2026-07-06
updated: 2026-07-06
source_path: 印象笔记管理工具/SOUL.md我不公开，但模板给你 2.md
...
“Helpful assistant” 不是操作模型。它没有告诉 Agent 什么最重要，什么时候该反对你，它有多少自主权，哪些事需要批准，遇到旧项目、模糊需求、错误假设、没人使用的输出时该怎么办。

一个严肃的 Agent，不只需要一个人设。

它需要角色、任务、边界、标准、行动权限，以及在你浪费时间时阻止你的权力。
...
目标是让你的 Agent 不再像一个被动文本框，而更像一个真正的 operator。

也就是说，它应该知道什么重要，能在事情不靠谱时反驳你，能区分事实和假设，能把高风险决策升级给你确认，也能在低风险任务上自己往前推。

但你必须定制它。

这点非常关键。

泛泛的 SOUL.md，只会得到泛泛的 operator。具体的 SOUL.md，才会给 Agent 一张真正能用的地图。
...
Agent 名字。 你的核心目标。 当前高优先级项目。 低优先级项目。 需要清理的地方。 私下沟通语气。 公开写作风格。 自主行动边界。 升级确认规则。

你写得越真实，它越有用。

如果你希望 Agent 挑战你，就写进去。 如果你讨厌它输出臃肿计划，就写进去。 如果你想让它提醒你哪些项目已经烂尾，也写进去。 如果你希望它帮你抵抗 shiny-object syndrome，更要写进去。
...
把这份内容放进你的 Agent 设置里。可以是上下文文件、system file、项目指令文件，或者你的框架支持的任何位置。

然后在使用中不断调整。

如果 Agent 太软，就加强 pushback 部分。 如果它总是问你要不要继续，就改清楚 autonomy 边界。 如果它产出的东西你根本不用，就优化 accountability 部分。 如果你的优先级变了，就更新 mission。 如果它写出来的东西不像你，就改 tone。
...
好的 SOUL.md，不是一次性写完的提示词，而是一份长期更新的操作契约。

[📎 在印象笔记中打开](evernote:///view/207087/s1/1bb0c565-10a9-4c10-b851-38eaff8e853c/1bb0c565-10a9-4c10-b851-38eaff8e853c/)
